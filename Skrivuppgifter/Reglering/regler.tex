...
Antag att en sensors riktiga värde ges av $x_true$, och att $\hat{x} $ är ett givet stickprov, alltså mätdata, med N antal mätningar. Alla tillstånd för $x_true$ antas vara lika sannolika, sensorvärdena är antas vara binomialfördelade. 

Sannolikheten att $x = x_true$ ges då av: 

\begin{gather}
P(X = x_{true}) = {n \choose k}p^{x_{true}}(1-p)^{n-x_{true}}
\end{gather}
\\
Där p är sannolikheten att sensorvärdet är $x_true$ och n är antalet mätningar. 
