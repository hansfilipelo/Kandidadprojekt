...
\begin{gather}
	\hat{x}_{n|n-1} = A_n\hat{x}_{n|n-1} + B_n(u_n) \\
	K_n = P_{n|n-1}C_{n}^T(C_{n}P_{n|n-1}C_{n}^T+R_2)^{-1}	\\
	P_{n+1|n}=A_{n-1}P_{n|n-1}A_{n-1}^T+R_{1} \\
...
Till skillnad från kalmanfiltret kan det utökade kalmanfiltret (EKF) användas för att approximativt lösa icke-linjära SLAM-problem. Det som möjliggör detta är att filtrets matriser beräknas om för varje ny tillståndsskattning. Matriserna för filtret beräknas med hjälp av jacobianer (9), och systemet linjäriseras därmed kring varje skattat tillstånd för att kunna skatta nästa tillstånd. Antag tillståndsrepresentationen av ett icke-linjärt system: 

\begin{gather}
	x_{t} = f(x_{t-1}, u_{t-1}) + e_{t-1}
	y_{t} = h(x{t},u{t}) + v{t-1}
\end{gather}

Det utökade kalmanfiltret är då en linjärisering och av systemet och skattning av dess tillstånd, enligt: 

\begin{gather}
	\hat{x}_{t} = f(\hat{x}_{t-1}, u_{t-1})
\end{gather}

Matriserna $K$ och $T$ i filtret, samt skattningen $\hat{x}$ ges sedan av:

\begin{gather}
	P_{t|t-1} = F_{t-1}P_{t-1}^T+R_{1},\;\;\;\;\;\; med \; F_{t} = \frac{\delta f(x,u)}{\delta x}\big|_{(x,u) = (\hat{x}_{t|t-1}, u_{t})} \\
	K_{t} = P_{t|t-1}H_{t}^T(H_{t}P_{t|t-1}H_{t}^T+R_{t})^{-1}\;\;\;\;\;\; med \; H_{t} = \frac{\delta H(x,u)}{\delta x}\big|_{(x,u) = (\hat{x}_{t|t-1}, u_{t})} \\
...
Matriserna $H$ och $F$ är alltså Jacobianer för systemets tillståndsmatriser och linjäriseras därmed i varje punkt.\footnote{David Törnqvist. Estimation and Detection with Applications to Navigation, s 31.}
...
Frågan som SLAM grundas på går nu att delvis besvara. Det går att kartlägga en omgivning utan att exakt veta sin position, men det behövs någon typ av skattning av positionen för att göra en rimlig kartläggning. Ju bättre skattningen är desto bättre kommer kartan bli i slutändan. Mer data och mer kvalitativ data bidrar till att förbättra skattningarna i kombination med olika typer av skattningsalgoritmer för att optimera och effektivisera processeringen av data. Problemet som kan uppstå då fler typer av sensorer används och större mängder data måste bearbetas är att beräkningskraften inte räcker till då filtren och mjukvaran som används för att implementera SLAM kan ställa för höga krav på beräkningsenheten.

\section{Reflektion}

Sett från det perspektiv vi granskar SLAM-problemet, inom ett kandidatprojekt, är avancerade FastSLAM-algoritmer för komplicerade för att implementera. Det måste dock alltid finnas någon typ av filter i en SLAM-implementering. Då beräkningar man gjort tidigare under kartläggningen ger konflikt med nuvarande mätning om huruvida en vägg faktiskt finns där eller ej. Enklare fall av filtrering kommer alltså behöva lösas vid vår framtida implementering. Vi kommer då att lösa det genom att låta roboten besöka området på nytt och ta ett beslut utifrån de nya mätdata vi får in. 
