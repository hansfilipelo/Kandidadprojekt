...
Ett problem med hjulsensorer är att de ej ger en helt korrekt hastighets- och distansskattning då hjulen ofta slirar mot underlaget. 
...
\begin{gather}
\dot{\hat{x^r_t}} = Ax^r_t + Bu_t + K(y_t - C\hat{x^r_t})
\label{equ:observer}
\end{gather}
...
\begin{gather}
\dot{\tilde{x^r_t}} = (A - KC)\tilde{x^r_t}
...
\begin{gather}
\dot{x^r_t}=Ax^r_t+Bu_t+e^i_t \\
y_t=Cx^r_t+Du_t+v_t
\end{gather}

Mätfelet i ekvation \ref{equ:observerError} ges då istället av: 
\begin{gather}
\dot{\tilde{x^r_t}} = (A - KC)\tilde{x^r_t} + e^i_t - Kv_t
\end{gather}

Då $e^i_t$ och $v_t$ antas vara vita brus kan man, genom att uppskatta störningarnas kovariansmatriser, beräkna mätfelets kovarians. Kovariansen hos mätfelet minimeras sedan genom att välja 
\begin{gather}
K = PC^{T}R_{2}^{-1}
\end{gather}
...
\newpage
\section{Exempel på andra implementeringar}
